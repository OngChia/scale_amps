...
!<<< 2026/06 T.Hashino updated
     ! porocity or area ratio of a crystal
     real(PS)                :: q_e_ic

     ! porocity or area ratio of a matured particle
     real(PS)                :: q_e_im
!>>> 2026/06 T.Hashino updated
...
!<<< 2026/06 T.Hashino updated
  function get_effect_area( IS,is_mod2,sh_type2,alen, clen, mode) result(A_e)
    ! +++ area projected from the top of the basal plane +++
    type (Ice_Shape), intent(in)    :: IS
    real(PS), intent(in)            :: alen, clen
    integer,intent(in)              :: is_mod2,mode,sh_type2
    real(PS)                        :: A_e
    ! use Chiruta and Wang (2003) for 4 lobes
    real(PS),parameter        :: C_ros=0.899 ! sense test ros original C_ros=0.899, changed to 0.744
    integer :: i_mode_eq1,i_sh_le1,i_sh_le2,i_habit_eq4, &
               i_ismod2_eq1,i_agtc,i_habit_le3
    real(PS) :: v_ros,v_obl,v_prl,v_hex,v_col
...
! [mod] Oct 2024 Hirata
    i_sh_le1=0.5*(1.0-real(isign(1,sh_type2-2),PS_KIND))
! end [mod] Oct 2024 Hirata

    i_habit_eq4=max(0.0,(-isign(1,IS%habit-4)*max(0,IS%habit-4)+1.0))*&
                max(0.0,(-isign(1,IS%habit-4)*min(0,IS%habit-4)+1.0))

    i_ismod2_eq1=max(0.0,(-isign(1,is_mod2-1)*max(0,is_mod2-1)+1.0))*&
                max(0.0,(-isign(1,is_mod2-1)*min(0,is_mod2-1)+1.0))

    i_agtc=0.5*(1.0-sign(1.0,clen-alen))
    ...
    A_e = & ! case of mode == 1
       ...
       ! case of mode==2
        (1.0-real(i_mode_eq1,PS_KIND))*&
          (&
          real(i_sh_le1,PS_KIND)*&
            (&
            real(i_habit_le3*i_ismod2_eq1,PS_KIND)*&
              (real(i_agtc,PS_KIND)*v_hex+&
              (1.0-real(i_agtc,PS_KIND))*v_col)+&
            real(i_habit_eq4*i_ismod2_eq1,PS_KIND)*v_ros+&
            (1.0-real(i_habit_le3*i_ismod2_eq1,PS_KIND))*(1.0-real(i_habit_eq4*i_ismod2_eq1,PS_KIND))*&
              (real(i_agtc,PS_KIND)*v_obl+(1.0-real(i_agtc,PS_KIND))*v_prl) &
            ) +&
          (1.0-real(i_sh_le1,PS_KIND))*&
            (real(i_agtc,PS_KIND)*v_obl+(1.0-real(i_agtc,PS_KIND))*v_prl)&
         )

  end function get_effect_area

  function get_circum_area( IS, is_mod2,sh_type2,alen, clen, mode) result(A_c)
    ! +++ area projected from the top of the basal plane +++
    type (Ice_Shape), intent(in)    :: IS
    real(PS), intent(in)               :: alen, clen
    integer,intent(in)                 :: is_mod2,mode,sh_type2
...
!
  ...
  subroutine diag_habit_v4t(level,tk,alen,clen,dlen,ag,cg,n_exice,&
       xlen_s1,xlen_c2a,xlen_s3,&
       rlen,elen,gam_ic,eta_ic,vice,semi_a_i,semi_c_i,habit)
    integer, intent(in) :: level
    real(PS), intent(in) :: tk ! temperature [K]
    real(PS), intent(in) :: alen,clen,dlen,ag,cg,n_exice
    real(PS), intent(in) :: xlen_s1,xlen_c2a,xlen_s3
    real(PS), intent(inout) :: rlen,elen,gam_ic,eta_ic,vice,semi_a_i,semi_c_i
    integer, intent(inout) :: habit
    ...
    i_agtc=0.5*(1.0-sign(1.0,clen-alen))
    i_a2o3_gtd=0.5*(1.0-sign(1.0,dlen-alen*2.0/3.0))
    i_ag_ge_cgclen=0.5*(1.0+sign(1.0,ag-(cg/clen+0.5)*alen))
    i_cg_ge_agalen=0.5*(1.0+sign(1.0,cg-(ag/alen+0.5)*clen))

!!!    i_ne_lt0p5=0.5*(1.0-sign(1.0,n_exice-0.5))
    ...
    if( n_exice<0.5) then
          ...
          vice=coef4pi3*( alen*alen+clen*clen)**1.5
          ...
          ! added for Oliktok
          ! diagnose the aspect ratio based on the local temperature
          spx_pt=max(0.4e+0,min(0.7e+0, &
                (0.7e+0-0.4e+0)/(257.0e+0-240.0e+0)*(tk-240.0e+0)+0.4e+0  ))
          ! end added for Oliktok
          elen=&
          ! side planes or radiating assemblage of plates: S1 or P7a
                real(i_ag_ge_cgclen,PS_KIND)*xlen_s1 +&
          ! bullet rosette
                (1.0_PS-real(i_ag_ge_cgclen,PS_KIND))*real(i_cg_ge_agalen,PS_KIND)*0.0  +&
          ! combination of side planes, bullets, and columns: S3
                (1.0_PS-real(i_ag_ge_cgclen,PS_KIND))*(1.0_PS-real(i_cg_ge_agalen,PS_KIND))*&
                  xlen_s3
          ...
          rlen=&
          ! side planes or radiating assemblage of plates: S1 or P7a
                real(i_ag_ge_cgclen,PS_KIND)*0.0 +&
          ! bullet rosette
                (1.0_PS-real(i_ag_ge_cgclen,PS_KIND))*real(i_cg_ge_agalen,PS_KIND)*&
                  xlen_c2a  +&
          ! combination of side planes, bullets, and columns: S3
          ...
          semi_c_i=&
          ! side planes or radiating assemblage of plates: S1 or P7a
                real(i_ag_ge_cgclen,PS_KIND)*elen*spx_pt +&
          ! bullet rosette
                (1.0_PS-real(i_ag_ge_cgclen,PS_KIND))*real(i_cg_ge_agalen,PS_KIND)*rlen*spx_pt +&
          ! combination of side planes, bullets, and columns: S3
                (1.0_PS-real(i_ag_ge_cgclen,PS_KIND))*(1.0_PS-real(i_cg_ge_agalen,PS_KIND))*rlen*spx_pt

          vice=coef4pi3*((1.0_PS+spx_pt**2.0)**1.5)
...
!          if(ag>=(cg/clen+0.5)*alen) then
!             ! side planes or radiating assemblage of plates: S1 or P7a
!             habit = 5
!             elen=get_len_s1(m_ice)
!             rlen=0.0
!
!             vice=coef4pi3*((1.0+spx_p**2.0)**1.5)*elen**3.0
!
!             semi_a_i=elen
!             semi_c_i=elen*spx_p
!
!          elseif(cg>=(ag/alen+0.5)*clen) then
!             ! bullet rosette
!             habit = 4
!             rlen=get_len_c2a(m_ice)
!             elen=0.0
!             vice=coef4pi3*((1.0+spx_p**2.0)**1.5)*rlen**3.0
!
!             semi_a_i=rlen
!             semi_c_i=rlen*spx_p
!
