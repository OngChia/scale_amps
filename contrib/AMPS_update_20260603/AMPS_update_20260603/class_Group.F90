...
!!c             g%MS(i,n)%CAP_hex = ( 0.708_PS + 0.615_PS * (g%IS(i,n)%phi_ic**(-0.76)))*g%MS(i,n)%c_len
   ...
   !!c             g%MS(i,n)%CAP = max(g%IS(i,n)%r,g%IS(i,n)%e)
          ...
          elseif( g%IS(i,n)%sh_type <= 3 ) then
            ...
            ! --- case of graupels ---
            phi = g%IS(i,n)%phi_cs
            if( phi < 1.0e+0-allow ) then
              ! --- in case of oblate spheroids ---
              d = sqrt( g%MS(i,n)%semi_a**2.0 - g%MS(i,n)%semi_c**2.0)
              eps = sqrt( 1.0_PS - phi**2.0)
              g%MS(i,n)%CAP = d/asin(eps)
            else if( phi > 1.0e+0+allow ) then
              ! --- in case of prolate spheroids ---
...
!<<< 2026/06 T.Hashino updated
          A_e = get_effect_area( g%IS(i,n),g%IS(i,n)%is_mod(2),g%IS(i,n)%sh_type, &
                                 g%MS(i,n)%semi_a, g%MS(i,n)%semi_c, mode)

          A_e_ic = get_effect_area( g%IS(i,n),1,1, &
                                    g%MS(i,n)%a_len, g%MS(i,n)%c_len, mode)

          ! ++++++ calculate the circumscribed cross-sectional area ++++++
          A_c = get_circum_area( g%IS(i,n),g%IS(i,n)%is_mod(2),g%IS(i,n)%sh_type, &
                           g%MS(i,n)%semi_a, g%MS(i,n)%semi_c, mode)

          A_c_ic = get_circum_area( g%IS(i,n),1,1, &
                           g%MS(i,n)%a_len, g%MS(i,n)%c_len, mode)
!>>> 2026/06 T.Hashino updated
...
!!c       end if
          ...
          if(g%MS(i,n)%vtm>5.0e+3.and.g%MS(i,n)%den>0.8_PS) then
           ! Y.SATO added condition on 20250422
             if(g%MS(i,n)%mass(1)>1.0e-10.or.g%MS(i,n)%con>1.0e-10) then
                ...
                LOG_WARN("cal_terminal_vel_vec",*) "WARNING: terminal_vel > The ice vel is set to 50.0m/s"
                LOG_WARN_CONT(*) "old vel",g%MS(i,n)%vtm
                LOG_WARN_CONT('("mass,con,alen,clen,q,k,gamma",10ES14.6)') g%MS(i,n)%mass(1),g%MS(i,n)%con,g%MS(i,n)%a_len,g%MS(i,n)%c_len,q,k,gamma
                LOG_WARN_CONT('("N_re0,ex,beta,X,C_DP",10ES14.6)') N_re_0,exp(-beta*gamma),beta,X,C_DP
                LOG_WARN_CONT('("semi_a,semi_c,mean_mass",10ES14.6)') g%MS(i,n)%semi_a,g%MS(i,n)%semi_c,g%MS(i,n)%mean_mass
                LOG_WARN_CONT('("char_len,d_vis,den,Nre,A_c",10ES14.6)') char_len,ag%TV(n)%d_vis,ag%TV(n)%den,g%MS(i,n)%Nre,A_c
                LOG_WARN_CONT('("pressure,T",10ES14.6)') ag%TV(n)%P,ag%TV(n)%T
             endif
