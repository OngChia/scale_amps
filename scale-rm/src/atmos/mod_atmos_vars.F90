...
module mod_atmos_vars
 ...
 ! tendency by physical processes
  ...
  integer, private              :: QH_MONIT_id(N_HYD)   !> hydrometeors
...
!OCL XFILL
    ...
    if ( moist ) then
       do iq = 1, N_HYD
          call MONITOR_reg( trim(HYD_NAME(iq))//"_hyd", HYD_DESC(iq), "kg", & ! (in)
                            QH_MONIT_id(iq),                                & ! (out)
                            dim_type='ZXY', is_tendency=.false.             ) ! (in)
       end do
    end if
  ...
  subroutine ATMOS_vars_restart_read(skip_reading_amps_ice)
    ...
    if ( restart_fid /= -1 ) then
       ...
       do iq = 1, QA
          LOG_INFO("ATMOS_vars_restart_read debug",*) '1: ', iq, skip_reading_amps_ice
    ...
    if ( ATMOS_sw_phy_mp ) call ATMOS_PHY_MP_vars_restart_read(DENS,RHOT,QTRC)
  ...
  subroutine ATMOS_vars_history
    ...
    if ( moist ) &
         call ATMOS_PHY_MP_vars_history( DENS_av(:,:,:), TEMP(:,:,:), QTRC_av(:,:,:,:), RHOT_av(:,:,:))
!    if ( .false. ) then
...
!OCL XFILL
  ...
  subroutine ATMOS_vars_calc_diagnostics
    ...
    if ( ATMOS_sw_dyn_DGM ) then
      call dyn_dg_vars%Get_vel_fv( U, V, W ) ! (out)
    else
...
!OCL XFILL
          ...
          do i = IS, IE
          do k = KS, KE
             RHOQ(k,i,j) = DENS_av(k,i,j) * Qe(k,i,j,iq)
          enddo
          enddo
          enddo
          !$acc end kernels

       call MONITOR_put( QH_MONIT_id(iq), RHOQ(:,:,:) )
    enddo
