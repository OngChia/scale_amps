...
This is the SCALE model implemented with AMPS microphysics scheme. This version is used for performing the idealized simulations of INP seeding in strato-cumulus clouds. 

The run script is stored in `cloudlab` folder.

The full data for the complete simulations can be found in Ong, C. R., & Zhang, H. (2026). Dataset of INP seeding simulations by SCALE-AMPS [Data set]. Zenodo. https://doi.org/10.5281/zenodo.20744736.

The SCALE model is described below.
...
- Pathes
* SCALE_MATHLIB_LIBS    : Path for math library (e.g., -qmkl=sequential or -L/path -llpack)
